...
xs_px = np.array([p[0] for p in raw_points])
ys_px = np.array([p[1] for p in raw_points])

df = pd.DataFrame({'x': xs_px, 'y': ys_px})
df.to_csv('tracked_trajectory.csv', index=False)